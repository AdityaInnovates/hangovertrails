...
export async function GET(request: Request) {
  ...
  if (location && !isLocation(location)) {
    return fail("INVALID_LOCATION", "Trips can only be filtered by Tawang, Ziro, Mechuka, or Anini.");
  }
  ...
  const filteredLocation = location && isLocation(location) ? location : undefined;
